...
# Import the file with the performance metrics created for a previous homework
import evaluation
...
def load_data(dir_path):
    ...
    try:
        ...
        print("Input file {} cannot be opened".format(file_name))
        sys.exit(1)
...
def obtain_performance_metrics(y_true, y_pred):
    ...
    In this function you might call the below functions:
    ...
    for i, metric in enumerate(PERF_METRICS):
        ...
        performance_fn = getattr(evaluation, "compute_%s" % metric)
...
if __name__ == '__main__':
    ...
    for k in range(int(args.mink), int(args.maxk) + 1):
        ...
        for i in range(0, X_test.shape[0]):
            # Get a prediction for the current data point
            y_pred[i] = knn_obj.predict(X_test[i, ])

        # Compute the performance metrics
        vec_perf = obtain_performance_metrics(y_test, y_pred)
